...
LICENSE                   # MIT
README.md
docs/screenshot.png       # user-provided asset — never generated by the agent
```
...
| 7 | Pages workflow + README + polish | Deployment-ready: `.github/workflows/deploy.yml` exists and is valid YAML; `npm run build` succeeds; the Vite base path matches the repo name; README and LICENSE are complete. Literal Pages liveness is manual (merge/push, then enable Pages with source set to GitHub Actions). |
